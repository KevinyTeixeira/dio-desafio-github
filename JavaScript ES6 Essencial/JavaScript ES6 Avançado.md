...
## :crown: > 9. Fetch, Async/Await e EventEmitter
...
Para mais informações sobre **Fetch**, [leia está página]([Usando Fetch - APIs da Web | MDN (mozilla.org)](https://developer.mozilla.org/pt-BR/docs/Web/API/Fetch_API/Using_Fetch)).
...
fetch('/data.json') // (url: "http:localhost:808/data.json"), se houver um único argumento, ele será interpretado como o elemento a ser procurado!

// O uso mais básico do fetch() acarreta em um argumento — a pasta do recurso que você deseja buscar — e retorna uma promessa contendo a resposta (a Response object). Ou conforme apresentada abaixo, a responseStream.

// A RESPONSE OBJECT é apenas uma resposta HTTP, não os elementos obtidos em sí. Portanto, para realmente receber os dados, teremos que tratar os arquivos recebidos.

    .then(responseStream => { // faz a requisição no diretório indicado pelo fetch
...
```javascript
// ERROR EXPLANATION
// CENÁRIO 3: simule um de escrita na requisição e analise como o erro lhe é apresentado;
...
ATENÇÃO! É importante observar o seguinte. O Catch só funcionou aqui por conta da tentativa do responseStream em transformar os dados obtidos em JSON. Se isso não estivesse incluído, o Catch nunca seria pego e nós não poderíamos tratar um erro de rede por exemplo!

O FETCH, ele só vai dar um erro na nossa PROMISSE caso seja um erro de rede, se for qualquer outro caso, ele executará ainda que não obtenha nenhum dado.
*/
```

```javascript
// CATCH ERRORS FIX
// CENÁRIO 4: certifique-se de que o código só será executado caso a requisição seja um sucesso. Se não, estruture o código para que o erro ocorrido possa ser recebido no catch!

fetch('http://localhost:8080/dataXPTO.json') // Endereço Errado!
	.then(responseStream -> {
    if (responseStream.status === 200) { // 200 = Sucesso!
...
// Agora o tratamento do erro ocorre como esperado!
```

```javascript
// FETCH ARGUMENTS
// CENÁRIO 5: FETCH possui outros parâmetros que podem ser passados como argumentos, veja abaixo.

/*
fetch(obrigatório, opcional);
fetch('pasta do recurso que você deseja buscar', 'configuração do request (por DEFAULT GET)');
*/

// Exemplo de configuração personalizada do request

var myInit = { method: 'post',
               body: JSON.stringify};

fetch('http://localhost:8080/data.json', myInit)
	.then(responseStream -> {
    if (responseStream.status === 200) { // 200 = Sucesso!
       return responseStream.json();
    } else {
...
`Async` é uma forma de criar promisses de maneira mais simples e permite trabalhar de forma mais ao utilizar um encadeamento de promisses.

```javascript
// ASYNC INTRO
// CENÁRIO 1: crie uma função simples utilizando ASYNC e observe seu comportamento;

/*
	O QUE FAZ? Transforma uma função em uma PROMISSE, permitindo trabalhar seu retorno com .then
	Para aplicar o ASYNC, basta escrever "async" antes de qualquer função!
	ARROW FUNCTION: simpleFunc = async () => {}
	NORMA FUNCTION: async simpleFunc() {}
*/
...
// RETORNO: Promise {<resolved>: 12345} | Retorna a promisse resolvida, portanto para receber os dados passamos a utilizar o .then!
```

```javascript
// CENÁRIO 2: verifique se, ao simular um erro dentro da função, este erro será recebido com um catch na estrutura da promisse;

const simpleFunc = async () => { 
	return 12345;
    throw new Error('Err: oh no!');
};
...
```javascript
//AWAIT
//CENÁRIO 3: simule a execução de duas promisses e aplique AWAIT observando seu comportamento;

// Faz com que uma promisse só seja executada após o término da execução de outra.

const asyncTimer = () => // Outra Promisse
	new Promise((resolve, reject) => {
        setTimeout(() => {
            resolve(12345);
        }, 1000);
...
const simpleFunc = async () => {
    const data = await asyncTimer(); // Executou esse primeiro
    console.log(data); // console.log
    const dataJSON = await fetch('/data.json').then(resStream => // fiz uma requisição
		resStream.json()
	);
...
12345
{data: Array(3)}
Fetch finished loading: GET "http://localhost:8080/data.json".
*/
```
...
É uma ferramenta exclusiva do Node.js, no qual deve importar o módulo `events`, através do código:

```javascript
const EventEmitter = require('events'); // importa o módulo events

const emitter = new EventEmitter(); // para utilizar, nós instanciamos o módulo
```

```javascript
// CENÁRIO 1: faça uso do EventEmitter;

const EventEmitter = require('events'); // importa o módulo events
const emitter = new EventEmitter(); // para utilizar, nós instanciamos o módulo

emitter.on('User logged', data => { // Criamos um consumidor desse evento
    console.log(data);
});

emitter.emit('User logged', { user: 'Celso Henrique'}) // E aqui, emitimos um evento
```

```javascript
// CENÁRIO 2: faça uso do EventEmitter, mas ao invés de instanciá-lo, use-o como extensão de uma classe;

const EventEmitter = require('events');

class Users extends EventEmitter { // Permite que a class Users utilize o módulo 'events'
    userLogged(data) { // O parâmetro data, fica responsável por receber as informações passadas por parâmetro
...
```javascript
// CENÁRIO 3: existe também o método chamado "once", que executa o evento somente uma vez. Faça o teste;
...
users.once('User logged', data => { // Ao usar "once", a execução deste evento só ocorre uma única vez
    console.log(data);
});
...
## :space_invader: > 10. Contexto de Programação
...
##### 10.1 Entenda o "Escopo" (`Scope`) e o "Contexto" (`Context`)
